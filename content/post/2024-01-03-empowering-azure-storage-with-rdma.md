...
一旦入口队列暂停上游设备，它就需要一个专用的余量缓冲区来吸收传输中的数据包，然后 PFC 暂停帧对上游设备生效 [50, 112]。理想的 PFC 余量值取决于许多因素，例如链路容量和传播延迟 [12]。交换机对净空缓冲区的总需求也与无损优先级的数量成正比。

为了将RDMA从集群规模[46, 50]扩展到区域规模，我们必须处理T2和RH（数十公里）之间以及T1和T2（数百米）之间的长链路，这需要比这大得多的PFC余量集群内链接。乍一看，我们生产环境中的 T1 交换机似乎可以为 PFC 余量和其他用途保留总缓冲区的一半。在 T2 和 RH，考虑到机箱交换机和长途链路的高端口密度（100 个），我们需要预留数 GB 的 PFC 净空缓冲区。

为了在长链路上扩展 PFC，我们利用了这样一个事实：病态情况（例如，所有端口同时拥塞，以及端口暂停对等点的入口无损队列）可能很少见。我们的解决方案有两个方面。首先，在 T2 和 RH 的机箱交换机上，我们使用片外 DRAM3 的深度数据包缓冲区来存储 RDMA 数据包。我们的分析表明，我们生产的机箱交换机可以为 PFC 余量提供充足的 DRAM 缓冲区。其次，我们不是为每个队列保留 PFC 余量，而是分配一个由交换机上所有入口无损队列共享的 PFC 余量池。每个入口无损队列都有一个静态阈值，以限制其在净空池中的最大使用量。我们以合理的比例超额认购净空池规模，从而留下更多的共享缓冲区空间来吸收突发。我们的生产经验表明，超额认购的PFC余量池可以有效消除拥塞损失，提高突发容忍度。

### 7.2 DCQCN 互操作性挑战
- - -

我们使用DCQCN[112]来控制每个队列对（QP）的发送速率。 DCQCN 由三个实体组成：发送方或反应点 (RP)、交换机或拥塞点 (CP) 以及接收方或通知点 (NP)。 CP基于RED算法[43]在出口队列处执行ECN标记。当 NP 收到带有 ECN 标记的数据包时，会发送拥塞通知数据包 (CNP)。当RP收到CNP时，它会降低其发送速率。否则，它会利用字节计数器和计时器来提高速率。

我们为不同类型的集群部署了来自流行 NIC 供应商的三代商品 NIC：Gen1、Gen2 和 Gen3。虽然它们都支持 DCQCN，但它们的实现细节却有很大差异。当不同代的 NIC 相互通信时，这会导致互操作性问题。

**DCQCN 实现差异：** 在 Gen1 上，大多数 DCQCN 功能（例如 NP 和 RP 状态机）在固件中实现。鉴于固件的处理能力有限，Gen1 通过 NP 侧的合并来最大限度地减少 CNP 的生成。如[112]中所述，如果该窗口内的任何到达数据包都被 ECN 标记，则 NP 在一个流的时间窗口中最多生成一个 CNP。相应地，RP在收到CNP后降低发送速率。此外，Gen1 的缓存资源也有限。缓存未命中会显着影响 RDMA 的性能 [50, 63]。为了减少缓存未命中，我们将 Gen1 的速率限制粒度从单个数据包增加到突发数据包。突发传输可以有效减少固定时间间隔内的活跃QP数量，从而减轻Gen1网卡非常有限的缓存资源的压力。

相比之下，Gen2和Gen3具有基于硬件的DCQCN实现，并采用基于RP的CNP合并机制，这与Gen1使用的基于NP的CNP合并机制完全相反。在 Gen2 和 Gen3 中，NP 为每个到达的带有 ECN 标记的数据包发送一个 CNP。然而，如果 RP 在某个时间窗口内收到任何 CNP，则它最多只会在该时间窗口内降低一次流的发送速率。值得注意的是，基于 RP 和基于 NP 的 CNP 合并机制本质上提供相同的拥塞通知粒度。速率限制针对 Gen2 和 Gen3 上的每个数据包粒度。

**互操作性挑战：** 跨不同集群的存储前端流量可能会导致不同代NIC之间的通信。在这种情况下，DC-QCN 实现差异会导致不良行为。首先，当 Gen2/Gen3 节点向 Gen1 节点发送流量时，其每包速率限制往往会触发许多缓存未命中在 Gen1 节点上，从而减慢接收器管道的速度。其次，当Gen1节点通过拥塞路径向Gen2/Gen3节点发送流量时，Gen2/Gen3 NP往往会向Gen1 RP发送过多的CNP，从而导致过度的速率降低和吞吐量损失。

**我们的解决方案：** 鉴于Gen1的处理能力和资源有限，我们无法使其表现得像Gen2和Gen3。相反，我们尝试让 Gen2 和 Gen3 的行为尽可能像 Gen1。我们的解决方案有两个方面。首先，我们将 Gen2 和 Gen3 上的 CNP 合并从 RP 侧移至 NP 侧。在Gen2/Gen3 NP方面，我们添加了per-QP CNP速率限制器，并将两个连续CNP之间的最小间隔设置为Gen1 NP的CNP合并计时器的值。在 Gen2/Gen3 RP 方面，我们最小化了速率降低的时间窗口，以便 RP 几乎总是在收到 CNP 时降低速率。其次，我们对 Gen2 和 Gen3 启用每次突发速率限制。

### 7.3 DCQCN 调优
- - -

当我们在 Azure 中调整 DC-QCN 时，存在某些实际限制。首先，我们的网卡只支持全局DCQCN参数设置。其次，为了优化客户体验，我们根据应用程序语义而不是 RTT 将 RDMA 流分类为两个交换队列。因此，我们没有对数据中心间和数据中心内流量使用不同的 DCQCN 参数，而是使用全局 DCQCN 参数设置（在 NIC 和交换机上），该设置在区域内 RTT 变化较大的情况下运行良好。

我们采用三步方法来调整 DCQCN 参数。首先，我们利用流体模型[113]来理解 DCQCN 的理论特性。其次，我们在实验室测试台上进行了合成流量实验，以评估互操作性问题的解决方案并提供合理的参数设置。第三，我们最终确定了测试集群中的参数设置，测试集群使用与承载客户流量的生产集群相同的设置。我们对真实存储应用程序进行了压力测试，并根据应用程序性能调整了 DCQCN 参数。

为了说明我们的发现，我们使用 Kmin、Kmax 和 Pmax 分别表示 RED/ECN [43] 的最小阈值、最大阈值和最大标记概率。我们做出以下三个关键观察（更多实验结果出现在§B中）：

* DCQCN 不会受到 RTT 不公平的影响，因为它是基于速率的协议，其速率调整独立于 RTT。

* 为了为具有大 RTT 的 DCQCN 流提供高吞吐量，我们使用具有大 Kmax − Kmin 和小 Pmax 的稀疏 ECN 标记。

* DCQCN 和开关缓冲器应该联合调整[112]。例如，在增加 Kmin 之前，我们确保无损流量的入口阈值足够大。否则，PFC 可能会在 ECN 标记之前触发。

## 8. 经验
- - -

2018 年，我们开始启用 RDMA 来服务客户后端流量。 2019 年，我们开始启用 RDMA 来服务客户前端流量，存储和计算集群位于同一数据中心。 2020 年，我们在第一个 Azure 区域启用了区域内 RDMA。截至 2023 年 2 月，Azure 公共区域中大约 70% 的流量是 RDMA（图 1），并且所有 Azure 公共区域都支持区域内 RDMA。

### 8.1 部署和服务
- - -

我们采取了三步方法逐步在生产环境中启用 RDMA。首先，我们利用实验室测试平台来开发和测试每个单独的组件。其次，我们在测试集群中进行了端到端的压力测试，其软硬件设置与生产集群相同。除了正常的工作负载之外，我们还注入了常见错误，例如随机丢包，以评估系统的稳健性。第三，我们谨慎增加生产环境中RDMA的部署规模，以承载更多的客户流量。在我们的部署过程中，NIC 驱动程序/固件和交换机操作系统更新很常见。因此，尽量减少此类更新对客户流量的影响至关重要。

**维护交换机：** 与 T1 或以上层中的交换机相比，T0 交换机（尤其是计算集群中的交换机）的维护更具​​挑战性，因为它们可能会成为客户虚拟机的单点故障 (SPOF)。在这种情况下，我们利用快速重启[17]和热重启[19]将数据平面中断时间从几分钟减少到不到一秒。

**维修 NIC：** 在某些情况下，维修 NIC 驱动程序或固件需要卸载 NIC 驱动程序。只有在所有网卡资源都被释放后，驱动程序才能安全卸载。为此，我们需要向消费者（例如磁盘驱动程序）发出信号，关闭 RDMA 连接并将流量转移到 TCP。一旦 RDMA 和其他具有类似问题的 NIC 功能被禁用，我们就可以重新加载驱动程序。

### 8.2 表现
- - -
**存储后端：** 目前Azure中几乎所有的存储后端流量都是RDMA。利用客户流量进行大规模 A/B 测试已不再可行，因为 RDMA 节省的 CPU 核心已被用于其他目的，更不用说客户体验下降了。因此，我们展示了 2018 年在测试集群中进行的 A/B 测试的结果。在此测试中，我们运行每秒事务数 (TPS) 高的存储工作负载，并在 RDMA 和 TCP 之间切换传输。图 7 绘制了存储的标准化 CPU 利用率两次传输交换机期间的服务器。值得注意的是，这里的 CPU 利用率包括所有类型的处理开销，例如存储应用程序、Azure 存储网络协议和 TCP/IP 堆栈。图 8 给出了在 Azure 存储网络协议层（图 4）中测量的消息完成时间，其中不包括应用程序处理的开销。与TCP相比，RDMA实现了明显的CPU节省并显着加速了网络数据传输。

**存储前端：** 由于我们无法对客户流量进行大规模A/B测试，因此我们展示了2018年在测试集群中进行的A/B测试结果。在本次测试中，我们使用DiskSpd在A处生成读写工作负载IOPS 和 B IOPS (A < B)。 I/O 大小为 8 KB。图 9 给出了测试期间主机域的平均 CPU 利用率。与 TCP 相比，RDMA 可以降低 CPU 利用率高达 34.5%。

为了了解 RDMA 带来的性能改进，我们利用始终在线的存储监控服务。该服务在每个区域分配一些虚拟机，使用它们定期生成磁盘读写工作负载，并收集端到端性能结果。监控服务涵盖不同的 I/O 大小、磁盘类型和存储前端流量的传输。

图 10 显示了监控服务一年来收集的所有 Azure 公共区域中某类 SSD 的总体平均访问延迟。请注意，该图中的 RDMA 和 TCP 仅指测试虚拟机生成的前端流量的传输。我们将 RDMA 结果标准化为相应的 TCP 结果。与 TCP 相比，RDMA 对于每种 I/O 大小都产生了更好的访问延迟。特别是，1 MB I/O 请求从 RDMA 中获益最多，读取和写入延迟分别减少了 23.8% 和 15.6%。这是因为，较大的 I/O 请求比较小的 I/O 请求对吞吐量更敏感，而 RDMA 可以极大地提高吞吐量，因为它可以使用单个连接以线速运行，而无需缓慢启动。

**拥塞控制：** 我们在测试集群中运行压力测试，以驱动 DCQCN 参数设置，即使在峰值工作负载下也能实现合理的性能。图 11 给出了第 99 个百分位消息完成时间的结果，这是我们用来指导调整的关键指标。一开始，我们禁用了 DCQCN，只调整了开关缓冲区参数 -例如，入口无损队列的动态阈值，以探索仅通过 PFC 实现的最佳性能。仅达到 PFC 的最佳性能后，我们使用默认参数设置启用 DC-QCN，该设置是使用合成流量在实验室测试台上导出的。虽然 DCQCN 减少了 PFC 暂停帧的数量，但它降低了尾部消息完成时间，因为默认设置过度降低了发送速率。鉴于此，我们调整了ECN标记参数以提高DCQCN的吞吐量。通过优化设置，DCQCN 的性能优于单独使用 PFC。我们从这次调整经验中得出的主要结论是，应该联合调整 DCQCN 和开关缓冲区来优化应用程序性能，而不是 PFC 暂停持续时间。
...
我们认为未来主机网络和物理网络应该融合。我们预计这个融合网络将是迈向分散云的重要一步。我们期待以与今天管理物理网络类似的方式来运营这个融合网络。
