...
SONiC 提供 RDMA 部署所需的所有功能，例如 ECN 标记、PFC、PFC 看门狗（第 3.1 节）和共享缓冲区模型。限于篇幅，我们简单介绍一下SONiC在披萨盒开关上的缓冲器模型和配置实践，用于T0和T1（§2.1）。我们提供了一个缓冲区配置示例 A。

我们通常在披萨盒交换机上分配三个缓冲池：（1）ingress_pool用于所有数据包的入口准入控制，（2）egress_lossy_pool用于有损数据包的出口准入控制，以及（3）egress_lossless_pool用于无损的出口准入控制数据包。请注意，这些缓冲池和队列并不由单独的专用缓冲区支持，而是本质上是应用于单个物理共享缓冲区并用于准入控制目的的计数器。每个计数器仅由映射到它的数据包更新，并且同一数据包可以同时映射到多个队列和池。例如，从源端口 s 到目标端口 d 的优先级为 p 的无损（有损）数据包会更新入口队列（s，p）、出口队列（d，p）、ingress_pool 和 egress_lossless_pool（egress_lossy_pool）。仅当数据包通过入口和出口准入控制时才会被接受。计数器递增按已接纳数据包的大小计算，并按离开数据包的大小递减。我们使用动态阈值[40]和静态阈值来限制队列长度。

我们仅对无损流量应用入口准入控制，并且仅对有损流量应用出口准入控制。如果交换机缓冲区大小为 B，则 ingress_pool 大小必须小于 B，为 PFC 净空缓冲区保留足够的空间（第 7.1 节）。当入口无损队列达到动态阈值时，队列进入“暂停”状态，交换机向上游设备发送 PFC 暂停帧。此入口无损队列上未来到达的数据包使用 PFC 净空缓冲区而不是 ingress_pool。相反，对于入口有损队列，我们​​配置一个等于交换机缓冲区大小 B 的静态阈值。由于入口有损队列长度无法达到交换机缓冲区大小，因此有损数据包可以绕过入口准入控制。

在出口处，有损和无损数据包分别映射到 egress_lossy_pool 和 egress_lossless_pool。我们将 egress_lossless_pool 的大小和出口无损队列的静态阈值配置为 B，以便无损数据包绕过出口准入控制。相反，egress_lossy_pool 的大小必须不大于 ingress_pool 的大小，因为有损数据包不应在入口处使用任何 PFC 余量缓冲区。出口有损队列配置为使用动态阈值 [40] 来丢弃数据包。

### 6.3 使用 SONiC 测试 RDMA 特性
- - -

我们每晚进行测试来跟踪 SONiC 开关的质量。在本节中，我们简要介绍使用 SONiC 交换机测试 RDMA 功能的方法。

**基于软件的测试：**我们利用数据包测试框架 (PTF) [10] 来开发 SONiC 的一般测试用例。 PTF主要用于测试数据包转发行为，测试RDMA功能需要额外的工作。

我们的测试方法受到软件调试中断点的启发。要为交换机设置“断点”，我们首先使用 SAI API 阻止交换机端口的传输。然后，我们生成一系列发往被阻止端口的数据包，并捕获一个或多个交换机状态快照（例如缓冲区水印），类似于在软件调试中转储变量值。接下来，我们释放端口并转储收到的数据包。我们通过分析捕获的交换机快照和接收到的数据包来确定测试是否通过。我们使用这种方法来测试缓冲区管理机制、缓冲区相关计数器和数据包调度程序。

**基于硬件的测试：**虽然上述方法使我们能够很好地了解交换机状态和数据包微行为，但它无法满足某些测试严格的性能要求。例如，为了测试PFC看门狗[50]，我们需要高速生成连续的PFC暂停帧，并由于暂停持续时间短而准确控制其间隔由每个 PFC 框架强制执行。

为了进行此类性能敏感的测试，我们需要以μs甚至ns的时间尺度控制流量生成，并对数据平面行为进行高分辨率测量。这促使我们利用硬件可编程流量生成器构建基于硬件的测试系统[9]。我们基于硬件的系统专注于测试 PFC、PFC 看门狗、RED/ECN 标记等功能。

截至 2023 年 2 月，我们为 RDMA 功能构建了 32 个软件测试用例和 50 个硬件测试用例。我们的测试用例的文档和实现可以在 [18] 中找到。
