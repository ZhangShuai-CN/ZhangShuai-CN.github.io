...
为了减少网络中RSP数据包的数量，提高ALM的效率，我们在ALM中采用批处理设计。在vSwitch中，我们允许将多个查询请求封装到单个RSP数据包中。相应地，网关也可以实现批量处理，将多个响应封装在一个回复包中。我们在部署中验证了我们的设计，得到的结果是平均请求数据包长度约为 200 字节，RSP 的最大带宽份额 < 4%（参见第 7.1 节）。因此，与它为虚拟网络带来的更强大的功能相比，这种开销是可以接受的（例如，我们可以在必要时通过 RSP 协议协商租户连接的 MTU、加密功能和其他功能）。
