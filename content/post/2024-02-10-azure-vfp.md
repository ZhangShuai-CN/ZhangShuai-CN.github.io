...
在图 9 中，我们首先看到，在 TCP SYN 情况下，§6.5 中的分类算法比随机分布的 rule 的线性匹配提高了 1-2 个数量级。测量长生存周期 flow 的 VFP 快速路径，我们接下来看到，当每 layer 包含 200 个 rule，UFT 缓存即使在只有一层的情况下也能提高性能，随着添加层数的增多，性能会显著增加。然后我们发现，由于 lock 的缘故，在 VFPv1 中，当我们扩展到 8 个 CPU 时，性能并没有提升，而 UFT 性能则可以很好地随 CPU 核数量扩展。最后，我们发现使用 PacketDirect，pps（packets per second）提高了 60% 以上。
...
* **一切都是 action（Everything is an action）**：将 VL2 风格的 encap/decap 建模为动作而不是隧道接口是一个不错的选择。它为所有数据包启用了单个表查找 - 无需遍历带有前后表的隧道接口。由此产生的HT语言将封装/解封与标头修改相结合，实现了单表硬件卸载。
