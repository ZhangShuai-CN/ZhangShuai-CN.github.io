...
让我们看一下哈希表中的值，即跟踪的连接，它们是作为 struct nf_conn 的实例实现的。该结构稍微复杂一些，拥有相当多的成员变量列表，甚至可以在运行时动态扩展，具体取决于第 4 层协议和几种可能的扩展。图 2 显示了包含成员变量的结构的简化视图，我认为这与本文的范围最相关。正如您所看到的，它包含一个引用计数器、处理到期的超时、对该连接所属的网络命名空间的引用、该连接的状态位的整数以及第 4 层协议特定的数据。对于 TCP，后者例如包含诸如跟踪的 TCP 序列号等内容。

![](/img/2024-01-27-linux-conntrack/figure2.2.png)

> 图 2.2：具有最相关成员变量的 struct nf_conn 的简化视图。

一个非常有趣的成员是 tuplehash[2] 数组，它是 struct nf_conn 的实例被集成到哈希表中的方法。如前所述，在哈希表中，有必要将键（此处为元组 struct nf_conntrack_tuple）保存为值（此处为 struct nf_conn）的一部分，因为哈希表中的查找只能通过比较键来进行，如果哈希有碰撞。然而，对于 ct 系统，这里还需要增加一点复杂性：必须能够在哈希表中查找沿原始数据流方向流动的网络数据包的跟踪连接，并且还必须能够在哈希表中查找跟踪的连接。沿回复方向流动的数据包。两者都会产生不同的元组和不同的哈希值，但查找最终仍必须找到 struct nf_conn 的相同实例。因此，tuplehash 是一个包含两个元素的数组。两个元素均包含一个元组和指针3)，以将该元素连接到哈希表的“桶”的链表。第一个元素（如图 2 中的绿色所示）代表原始方向。其元组的成员方向设置为 0 (orig)。第二个元素（如图 2 中的红色所示）表示回复方向。其元组的成员方向设置为1（回复）。当然，每个被跟踪的连接只存在一个 struct nf_conn 实例，但是该实例被添加到哈希表中两次，一次用于原始方向，一次用于回复方向，以便可以查找两种类型的数据包。我将在下面详细描述这一点。

### 2.4 查找现有连接
- - -

让我们详细介绍一下连接查找。在图 3 中，TCP 数据包正在遍历 Netfilter 预路由钩子 4) 中的 ct 钩子函数。该钩子函数的大部分有趣部分是在函数 nf_conntrack_in() 中完成的。在其他函数中，它调用resolve_normal_ct()，这就是查找发生的地方。图 3 详细显示了这一点。在此示例中，我假设 TCP 数据包所属的连接此时已被 ct 系统已知并跟踪。换句话说，我假设这不是 ct 系统看到的该连接的第一个数据包。

![](/img/2024-01-27-linux-conntrack/figure2.3.png)

> 图 2.3：详细查找跟踪连接。

在步骤 (1) 中，调用函数 nf_ct_get_tuple() 从 TCP 数据包创建一个元组，即哈希表的键（与图 1 中所示的内容相同）。在这种情况下，元组的成员方向始终设置为 0 (orig)。它在这里没有实际意义，因为此时 ct 系统尚不知道 TCP 数据包是否是所跟踪连接的原始方向或回复方向的一部分。在步骤(2)中，调用函数hash_conntrack_raw()根据元组的成员变量值计算哈希值。成员方向不包含在哈希计算中，如图 3 中的虚线矩形所示，原因刚刚解释过。但是，还额外包含了提到的网络命名空间的种子 hash_mix。现在在哈希表中的实际查找是通过步骤(3)、(4)和(5)中的函数__nf_conntrack_find_get()执行的：哈希值在步骤(3)中用作哈希表数组中的索引来定位正确的存储桶，如图 3 中的橙色所示。在此示例中，该存储桶已包含 struct nf_conn 的三个实例。换句话说，这里跟踪的三个连接已经为此哈希发生冲突。在步骤 (4) 中，代码现在迭代此存储桶的链表，并将每个连接实例的成员 tuplehash[] 内的元组与步骤 (1) 中从 TCP 数据包生成的元组进行比较。如果元组的所有成员变量都匹配（成员方向除外，这里再次忽略）并且网络命名空间匹配，则连接被视为匹配。在步骤 (5) 中找到匹配项。我在图 3 中用 X 标记了匹配实例，以表明它存在于哈希表中的两个不同位置。在一个地方，它用它的 tuplehash[0] 成员的指针连接到桶链表（原始方向，以绿色显示），而在另一处，它用它的 tuplehash[1] 成员的指针连接（回复方向） ，以红色显示）。正如您所看到的，在步骤（5）中匹配的是tuplehash[0]（绿色）。这意味着有问题的 TCP 数据包是该跟踪连接的原始方向的一部分。在步骤(6)中，调用函数nf_ct_set()来初始化网络数据包的skb->_nfct，使其指向刚刚找到的匹配的struct nf_conn5)实例。最后，在数据包完成遍历 ct 挂钩函数之前，正在检查数据包的 OSI 第 4 层协议（在我们的示例中为 TCP）6)。
